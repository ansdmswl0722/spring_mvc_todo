...
const apiStore = function(){
    ...
    */
    api.save= async function(todoDate, todoSubject){
        ...
        */
        ...
        const save = await fetch(url,options).then(response =>{
            if(response.ok) {
                console.log("저장완료 - " + JSON.stringify(data));
    ...
    */
    api.delete = async function(todoDate,id){
        ...
        if(!response.ok){
            throw new Error("삭제 api error!! code: " + response.status);
        }
    ...
    */
    ...
    api.getTodoItemList= async function(todoDate){
        ...
        if(!response.ok){
            throw new Error("조회 api error!! state: " + response.status);
        }

        const json = await response.json();
        const items = [];
        if(json){
            for(const todoItem of json){
                const item = {
                    'id' : todoItem.id,
                    'todoDate' : todoItem.eventAt,
                    'todoSubject': todoItem.subject
                }
                items.push(item);
                // console.log(items);
            }
